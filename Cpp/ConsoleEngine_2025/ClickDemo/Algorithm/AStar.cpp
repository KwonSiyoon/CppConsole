...
std::vector<Node*> AStar::FindPath(Actor* startActor, Actor* endActor)
{
    
    startNode = new Node(startActor->Position());
    goalNode = new Node(endActor->Position());
...
bool AStar::IsInRange(int x, int y, Node* startNode, Node* goalNode)
{
    if (x < 0 || y < 0)
    {
        return false;
    ...
    int maxX = startNode->position.x <= goalNode->position.x ? goalNode->position.x : startNode->position.x;
    int minX = startNode->position.x >= goalNode->position.x ? goalNode->position.x : startNode->position.x;
